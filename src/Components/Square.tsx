...
function Square({index,onClick,player}:Props){
  ...
  const textColor = player ==="X" ? "text-yellow-200" : "text-fuchsia-500"
  ...
  return(
    ...
    className={`h-36 border-solid border-4 border-slate-200 font-teste text-7xl text-center flex justify-center items-center cursor-pointer ${hoverStyle}`}
      {...{onClick }}
    >
      <span 
      data-cell-index={index}
